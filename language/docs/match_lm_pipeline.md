...
### Why Welford (and Why This Yields Pearson Correlation)

We use Welford’s algorithm to compute mean and variance **incrementally** and **numerically stably**. Directly computing variance as `E[x^2] - E[x]^2` is prone to catastrophic cancellation when `E[x^2]` and `E[x]^2` are close (common with large sample counts). Welford avoids that by maintaining:

- `count` (or sum of weights for spans)
- `mean`
...
This is accurate for large sample counts and stable for distributed aggregation because M2 values can be combined with a correction term.

**Why this is correct for Pearson correlation**:

Pearson correlation between vectors `x` and `y` is:

```
corr(x, y) = E[(x - mean_x) / std_x * (y - mean_y) / std_y]
```

So if we:
1. Compute `mean_x`, `std_x`, `mean_y`, `std_y` accurately (Welford),
2. Standardize each sample by those stats,
3. Average the outer products across samples,

then the result is exactly the Pearson correlation matrix (up to floating-point error).

#### Distributed Welford (Why It’s Correct)

In distributed runs, each rank computes local `(sum_w, mean, M2)` for its shard. These can be merged exactly (up to float error) without revisiting raw samples:

- Let local stats be `(sum_w_i, mean_i, M2_i)` for rank `i`.
- The global mean is the weighted average:

```
mean_global = sum(sum_w_i * mean_i) / sum(sum_w_i)
```

- The global M2 is:

```
M2_global = sum(M2_i) + sum(sum_w_i * (mean_i - mean_global)^2)
```

This correction term accounts for differences between local means and the global mean. The result is identical to running Welford on the concatenated data stream, which is why the pipeline can safely all-reduce per-rank stats.
